...
As always, we need to build on prior art and don't start from scratch. In this case we need the logging and the string list (to keep all our files, that we want to process). The string list needs `str.h`, but we don't use it directly here, so we wont include it.
...
The system headers are good old friends, with the exception of `dirent.h`. We use it to parse the directory, if `content.txt` is not provided.
...
We have two ways to get the files. The preferred way is to provide a `content.txt` file.  It contains a list of files. If we can't read this file, we will log an error! But to make things easy, we will read all `*.c`, `*.h`, and `*.md` files (without `README.md`) as a backup strategy.

The `context.txt` is clearly the preferred way, because you can provide the correct order of the files.

The code is simple: try the `content.txt` and if it fails, load the directory list (which may also fail).
...
The suffix check contains a grain of magic. We can use good ol' `strcmp` to perform the check, if we know where to start it. We need to shift the candidate by its length minus the suffix length.
...
The reverse comparator is simply the reverse of `strcmp`. We could also exchange the parameters, but I prefer the negation.
...
The kind of the file sets the first flag, and we always start writing in MarkDown mode.
...
The reading looks like the reading of `context.txt`, but the lines can be much longer. Some MarkDown tools create a line per paragraph. Maybe we need a better method, but for a start 10k seems enough.
...
If a code line starts with the special comment, we start reading MarkDown that is inside the comment.
...
If we are reading MarkDown in code and a line starts with the special ending comment, we stop reading MarkDown and switch back to code.
...
If we have read a line of code, but are in MarkDown writing, we send a MarkDown command to switch to code.
